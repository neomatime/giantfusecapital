...
**New file `assets/js/preloader.js` hides it.** It follows the established IIFE-wrapped-file shape, but deliberately does NOT expose a `window.Giantfuse.Preloader` namespace or `module.exports` — there's no callable API for anything else to invoke and no pure logic worth Node-testing, so both would be dead ceremony, and an exported `init()` specifically would reintroduce the race described below. This is one deliberate deviation from how every other page-specific script on this site works:
